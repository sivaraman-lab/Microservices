...
import com.creditcardapplication.config.AttributeEncryptor;
import com.creditcardapplication.config.FallBackConfig;
...
public class CreditCardApplicationServiceImpl implements CreditCardApplicationService {
	...
	public String applyCreditCard(RequestBean request) {
		...
		ApprovalResponse response = config.apiCallforapproval(approvalRequest);
		if(response==null) {
		...
		triggerMailToCustomer(request, customerId);
		return customerId;
	}
	...
	/*
	 * this method is used to trigger mail to customer
	 */
	private void triggerMailToCustomer(RequestBean request, String customerId) {
		try {
